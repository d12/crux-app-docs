...
def generate_webhooks_section(docs_json_data)
  events = docs_json_data["webhook_events"] || {}
  section = "<!-- BEGIN AUTOGENERATED WEBHOOKS -->\n\n"

  # Group events by owner type
  events_by_owner = events.group_by { |_, meta| meta["owner_type"] }

  events_by_owner.sort.each do |owner_type, owner_events|
    section << "## #{owner_type.capitalize} Events\n\n"

    owner_events.sort_by { |_, meta| meta["name"] }.each do |event_class_name, metadata|
      name = metadata["name"]
      desc = metadata["description"]
      payload_type = metadata["payload_type"]
      payload_link = "[#{blueprint_object_to_string(payload_type)}](/docs/api-documentation/api-reference.md##{blueprint_object_to_link(payload_type)})"

      section << "#### `#{name}`\n>\n"
      section << ">#{desc}\n>\n"
      section << ">Returns a [WebhookDelivery](/docs/api-documentation/api-reference.md#webhookdelivery) object with a `payload` field of type #{payload_link}.\n>\n"
    end
  end

  section << "<!-- END AUTOGENERATED WEBHOOKS -->"
  section
end
...
# Load and update the webhooks.md file
webhook_file_path = File.join(File.dirname(__FILE__), '../docs/api-documentation/webhooks.md')
webhook_file = File.read(webhook_file_path)

webhook_start = "<!-- BEGIN AUTOGENERATED WEBHOOKS -->"
webhook_end = "<!-- END AUTOGENERATED WEBHOOKS -->"
webhook_start_index = webhook_file.index(webhook_start)
webhook_end_index = webhook_file.index(webhook_end)

if webhook_start_index.nil? || webhook_end_index.nil?
  abort("❌ Could not find webhook markers in webhooks.md.")
end

webhook_section = generate_webhooks_section(docs_json_data)
webhook_file = webhook_file[0...webhook_start_index] + webhook_section + webhook_file[(webhook_end_index + webhook_end.length)..]

File.write(webhook_file_path, webhook_file)
puts "✅ Webhook documentation updated successfully."
