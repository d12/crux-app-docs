...
def generate_webhooks_section(docs_json_data)
  ...
  events.sort_by { |_, meta| meta["name"] }.each do |event_class_name, metadata|
    name = metadata["name"]
    desc = metadata["description"]
    owner = metadata["owner_type"]
    payload_type = metadata["payload_type"]
    payload_link = "[#{blueprint_object_to_string(payload_type)}](##{blueprint_object_to_link(payload_type)})"

    section << "### `#{name}`\n\n"
    section << "**Owner Type:** `#{owner}`\n\n"
    section << "#{desc}\n\n"
    section << "**Payload:** #{payload_link}\n\n"
  end
